...
def get_episode_files(episode_num: str, media_files: List[Path]) -> List[Path]:
    return sorted([f for f in media_files if f'- {episode_num} ' in f.name])
...
def merge_files(mkv_file: Path, audio_files: List[Path], sub_files: List[Path], output_dir: Path):
    ...
    for audio in audio_files:
        command += ['--track-name', f'0:{audio.parent.name}', str(audio)]
    
    for sub in sub_files:
        lang = sub.parent.name.replace(' ', '_')
    ...
    try:
        ...
        print(f"✗ Ошибка при обработке {mkv_file}: {e.stderr}")
        if temp_file.exists():
            temp_file.unlink()
...
def process_target(target_path: Path):
    ...
    audio_files = find_media_files(root_dir / "Sound", ['.aac'])
    ...
    for mkv in mkv_files:
        ...
        if (ep_num := extract_episode_number(mkv.name)):
